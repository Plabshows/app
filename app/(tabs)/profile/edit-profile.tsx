    ...
    const handleFinalSave = async () => {
        ...
        try {
            ...
            const profileUpdate: any = {
                ...
                group_members: formData.members || [],
                experience_awards: formData.awards || [],
            };
            if (firstPhoto) {
                profileUpdate.avatar_url = firstPhoto;
            }
            
            console.log('[Wizard] Saving to profiles for ID:', user.id);
            ...
            if (profileError) {
                console.error('[Wizard] Profile update error:', profileError);
                throw profileError;
            }
            ...
            const actPayload: any = {
                owner_id: user.id,
                name: formData.act_name || formData.full_name || null,
                description: formData.bio || null,
                category_id: formData.category_id || (formData.category_ids.length > 0 ? formData.category_ids[0] : null),
                ...
                group_members: formData.members || [],
                packages: formData.packages || [],
                location_base: formData.city || null,
                travel_departure_city: formData.city || null,
            };
            ...
            if (act) {
                ...
                await supabase.from('act_faqs').delete().eq('act_id', act.id);
                if (formData.faq.length > 0) {
                    await supabase.from('act_faqs').insert(
                        formData.faq.map(f => ({ ...f, act_id: act.id, id: undefined }))
                    );
                }
            }

            await refreshAuth();
            Toast.show({ type: 'success', text1: 'Guardado', text2: 'Tu perfil ha sido actualizado.' });
