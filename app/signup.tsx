...
export default function SignupScreen() {
    ...
    const handleSignup = async () => {
        ...
        try {
            ...
            if (signUpError) {
                console.error('[Signup] Error en Auth.signUp:', signUpError.message, signUpError);
                throw signUpError;
            }
            ...
            const errorMsg = error.message || 'Error desconocido durante el registro.';

            if (Platform.OS === 'web') {
                window.alert('Error de Registro: ' + errorMsg);
            } else {
                Alert.alert('Error de Registro', errorMsg);
            }
        } finally {
            setLoading(true); // Keeping loading true briefly to prevent double taps during redirect
