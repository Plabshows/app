...
export const PhotoStep = ({ data, updateData, onNext }: StepProps) => {
    ...
    const removePhoto = async (index: number) => {
        ...
        try {
            // URL format: .../storage/v1/object/public/media/USER_ID/FILENAME?t=...
            const pathParts = photoUrl.split('/media/');
            if (pathParts.length > 1) {
                const fullPath = pathParts[1].split('?')[0];
                const decodedPath = decodeURIComponent(fullPath);
                console.log('[PhotoStep] Deleting from storage:', decodedPath);
                const { error } = await supabase.storage.from('media').remove([decodedPath]);
                if (error) console.error('[PhotoStep] Storage deletion error:', error);
            }
        } catch (error) {
            console.error('[PhotoStep] Failed to parse URL for deletion:', error);
        }
    };
